...
from .functions import recommend, recommend_movie, generate_user_profile, clean_text, toxicity_prediction
...
def recommend_movies(request):
    ...
    if user_profile is not None:
        movies_id = recommend_movie(user_id, user_profile)
        movies = Movies.objects.filter(id__in=movies_id)

    else:
        referer = request.META.get('HTTP_REFERER', '/')
        messages.info(request, f"Sorry! No content available for {request.user}")
...
def movie(request, movie_id):
    ...
    if request.method == 'POST':
        ...
        if comment_form.is_valid():
            ...
            value = toxicity_prediction(clean_comment)
            print(value)
            
            if value[0] > 0.70:
                messages.info(request, 'Sorry, your comment voilets our rules!')
